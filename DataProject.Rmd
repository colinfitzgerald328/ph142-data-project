...
```{r}
library(janitor)
observed_table <- matrix(c(173, 141, 156,2,323,145,103, 99,146,195,295,1,145,187), nrow = 2, ncol = 7, byrow = T)
rownames(observed_table) <- c('Female', 'Male')
colnames(observed_table) <- c('Insufficient Weight', 'Normal Weight', 'Obesity Type I', 'Obesity Type II','Obesity Type III','Overweight Level I','Overweight Level II')
ObservedVal_Table <- knitr::kable(observed_table, caption = "Observed Data")
ObservedVal_Table
ExpectedVal_Table <- knitr::kable(chisq.test(observed_table)$expected, caption = "Expected Data")
ExpectedVal_Table
```
5. [2 points] Clearly state the null and alternative hypotheses for this test. 
Ho: Gender and obesity level are independent. 
Ha: Gender and obesity level are not independent. 
